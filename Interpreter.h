...
// Parser functions
Node parseExp(std::vector<Token>& tokens);
Node parseTerm(std::vector<Token>& tokens);
Node parseFact(std::vector<Token>& tokens);
Node parseIdentifier(std::vector<Token>& tokens);
Node parseLiteral(std::vector<Token>& tokens);
...
{
public:
    void interpret(const std::string& program) {
...
private:
    ...
    Node parseProgram(std::vector<Token>& tokens) {
        Node program;
    ...
    Node parseAssignment(std::vector<Token>& tokens) {
        ...
        if (tokens.front().type == TokenType::OPERATOR && tokens.front().value == "=") {
            ...
            throw std::invalid_argument("Expected assignment operator");
        }
    ...
    void execute(const Node& program) {
        for (const auto& assignment : program.children) {
            std::string identifier = assignment.children[0].value;
